...
namespace FolderProtector
{
    public partial class Main_Window : AntdUI.Window
    {
        ...
        {
            //锁定文件夹
            public static bool LockFolder(string folderPath)
            {
                try
                {
                ...
                {
                    MessageBox.Show($"锁定失败: {ex.Message}");
                    return false;
                }
            }

            //解锁
            public static bool UnlockFolder(string folderPath)
            {
                try
                {
                    var dirInfo = new DirectoryInfo(folderPath);
                    ...
                    // 移除匹配的拒绝规则
                    dirSecurity.RemoveAccessRuleSpecific(denyRule);
                    dirInfo.SetAccessControl(dirSecurity);
                    return true;
                }
                catch (Exception ex)
            ...
            //隐藏
            public static bool HideFolder(string folderPath)
            {
                try
                {
                    // 检查文件夹是否存在
                ...
                {
                    MessageBox.Show($"隐藏失败: {ex.Message}");
                    return false;
                }
            }

            //取消隐藏
            public static bool ShowFolder(string folderPath)
            {
                try
                {
                    // 检查文件夹是否存在
                ...
                {
                    MessageBox.Show($"显示失败: {ex.Message}");
                    return false;
                }
            }

            //受系统保护
            public static bool AddSystemProtection(string folderPath)
            {
                try
                {
                    // 检查文件夹是否存在
                ...
                {
                    MessageBox.Show($"添加系统保护失败: {ex.Message}");
                    return false;
                }
            }

            //取消保护
            public static bool RemoveSystemProtection(string folderPath)
            {
                try
                {
                    // 检查文件夹是否存在
                ...
                {
                    MessageBox.Show($"移除系统保护失败: {ex.Message}");
                    return false;
                }
            }
        }
        ...
        private void button_FolderPathBrowser_Click(object sender, EventArgs e)
        {
            ...
            if (temp != null) 
            {
                ...
                {
                    switch_Hide.Checked = true;
                }
                else
                {
                    switch_Hide.Checked = false;
                }

                if (Convert.ToInt32(ReadRegistryValue(Registry.CurrentUser, $"{RegConifgPath}\\{FolderPath}", "LockState")) == 1)
                {
                    switch_Lock.Checked = true;
                }
                else
                {
                    switch_Lock.Checked = false;
                }
        ...
        {
            this.Visible = false;
            using(Password_Window password_Window=new Password_Window())
            {
                if (password_Window.ShowDialog() == DialogResult.OK)
                {
                    this.Visible = true;
                }
                else
        ...
        private void input_FolderPath_Enter(object sender, EventArgs e)
        {

        }

        private void input_FolderPath_Leave(object sender, EventArgs e)
        {
